...
def test_append_elem_many_fast():
    ...
    assert len(graph._obj.elems) == 200
    assert len(graph._obj.group_id) == 200
    assert len(graph._obj.coords) == 200
    assert elapsed < 0.5
...
def test_add_collision_pair_symmetric():
    ...
    assert 1 in graph._obj.collisions[0]
    assert 0 in graph._obj.collisions[1]
    assert 2 in graph._obj.collisions[1]
    assert 1 in graph._obj.collisions[2]
