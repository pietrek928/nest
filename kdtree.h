...
#define CONST_SWITCH_5(expr, var_name, ...) switch(expr) { \
    case 0: {constexpr static int var_name = 0; __VA_ARGS__;} break; \
    case 1: {constexpr static int var_name = 1; __VA_ARGS__;} break; \
    case 2: {constexpr static int var_name = 2; __VA_ARGS__;} break; \
    case 3: {constexpr static int var_name = 3; __VA_ARGS__;} break; \
    case 4: {constexpr static int var_name = 4; __VA_ARGS__;} break; \
    case 5: {constexpr static int var_name = 5; __VA_ARGS__;} break; \
}
...
class KDTree {
    ...
    split_score_t score_split(int* items, int n, Tpos* buf /*size: 2n*/) {
        ...
        do {
            if (*bufb < *bufe) {
                ...
                double new_score = std::log2(max_d + 2.) + std::log2(insides + 2.) * 0.2;
    ...
    int split_and_build(
        ...
        *m = -1;
        ...
        tree[tree_node_n].left = build(items_buf, items, items_1 - items_buf, buf);
        tree[tree_node_n].right = build(items_2, items, items_buf_e - items_2, buf);
    ...
    int build(int* items, int* items_buf, int n, Tpos* buf /*size: 2n*/) {
        ...
        CONST_SWITCH_5(
            best_split.dim, K,
            return split_and_build<K>(
                items, items_buf, n, buf, best_split.pos
                )
        )
...
public:
    ...
    void match_point(std::vector<int>& out, const Tpos_lookup& pos) {
        ...
        do {
            ...
            if constexpr (ndims == 1) {
                ...
                node_n = dim_pos < tree_node.v ? tree_node.left : tree_node.right;
    ...
    int max_calc_path(int o = 0) {
        ...
        return std::max(max_calc_path(tree[o].left), max_calc_path(tree[o].right)) + list_cnt + 1;
    }
