...
void bind_graph_decision(nb::module_ &m) {
        ...
        .def(
            ...
            [](DecisionGraph &dg, const std::vector<int> &selected) {
                ...
                for (int i = 0; i < 4; ++i) {
                    ...
                    kind_sum += st.kind_count[i];
                }
                out["kind_survive"] = kind_survive;
                out["kind_survive_hist"] = kind_survive;
                out["kind_survive_n"] = kind_sum;
                return out;
            },
            nb::arg("selected"))
    ...
    nb::class_<MotifCohortSpec>(m, "MotifCohortSpec")
        .def(nb::init<>())
        .def_rw("motif_id", &MotifCohortSpec::motif_id)
        .def_rw("leader_gid", &MotifCohortSpec::leader_gid)
        .def_rw("member_keys_count", &MotifCohortSpec::member_keys_count);

    m.def("region_to_zone", &region_to_zone, nb::arg("region"));
    m.def("zone_to_region", &zone_to_region, nb::arg("zone"));
    m.def(
        "niche_amaf_key",
        [](nb::object action) -> std::tuple<int32_t, int32_t, int32_t> {
            if (action.is_none()) {
                return niche_amaf_key(nullptr);
            }
            const MacroAction &a = nb::cast<MacroAction &>(action);
            return niche_amaf_key(&a);
        },
    ...
    m.def(
        ...
        [](const DecisionGraph &dg,
           ...
           nb::dict survive_by_motif) {
            std::unordered_map<int32_t, int32_t> sc;
            for (auto item : survive_by_motif) {
                sc[nb::cast<int32_t>(item.first)] = nb::cast<int32_t>(item.second);
            }
            return rank_motif_join_neighbors(dg, macro_node_id, sc);
        },
        nb::arg("dg"),
        nb::arg("macro_node_id"),
        nb::arg("survive_by_motif") = nb::dict{});
    ...
    m.def(
        ...
        nb::arg("free_kind") = std::string{},
        nb::arg("motif_cohorts") = std::vector<MotifCohortSpec>{},
        nb::arg("blocked_action"));

    m.def(
        "survive_rank",
        [](const MacroAction &action, nb::dict realized) {
            MacroPathRealized r;
            if (realized.contains("survive_by_motif")) {
                const nb::dict sc = nb::cast<nb::dict>(realized["survive_by_motif"]);
                for (auto item : sc) {
                    r.survive_by_motif[nb::cast<int32_t>(item.first)] =
                        nb::cast<int32_t>(item.second);
                }
            }
            if (realized.contains("survive_motif_n")) {
                r.survive_motif_n = nb::cast<int32_t>(realized["survive_motif_n"]);
            }
            return survive_rank(action, r);
        },
        nb::arg("action"),
        nb::arg("realized") = nb::dict{});
}
