...
from nest_graph.elem_graph import MacroRegion, PathKind, node_macro
...
def _sibling_actions(
    ...
    *,
    ...
    actions = generate_macros(
        ...
        prefer_motifs=True,
        warm_motif_ids=agent._warm_motif_ids(snapshot) if snapshot else (),
        free_kind=free_kind,
        motif_cohorts=getattr(agent, "motif_cohorts", None),
    )
    blocked_key = agent._action_key(blocked_action)
    return [a for a in actions if agent._action_key(a) != blocked_key]


def _survive_rank(agent: Any, action: Any) -> float:
    """Q385: soft prefer high survive_counts / realized MotifJoins."""
    realized = getattr(agent, "realized", None) or {}
    sc = realized.get("survive_by_motif") or {}
    mid = int(getattr(action, "motif_id", -1) or -1)
    base = float(sc.get(mid, 0) or 0) if mid >= 0 else 0.0
    base += 0.01 * float(realized.get("survive_motif_n", 0) or 0)
    return base
...
def _path_neighbors_ranked(runner: Any, macro_node_id: int, agent: Any) -> list[Any]:
    ...
    if dg is None or not hasattr(dg, "neighbors"):
        return []
    try:
        step = node_macro(int(macro_node_id))
        nbrs = list(dg.neighbors(step) or ())
    except Exception:
        return []
    join_steps = [n for n in nbrs if getattr(n, "kind", None) == PathKind.MotifJoin]
    realized = getattr(agent, "realized", None) or {}
    sc = realized.get("survive_by_motif") or {}
    join_steps.sort(
        key=lambda n: (
            -int(sc.get(int(getattr(n, "motif_id", -1) or -1), 0) or 0),
            int(getattr(n, "motif_id", -1) or -1),
        )
    )
    return join_steps
...
def _validate_join_bonus(agent: Any, join_step: Any, baseline: float) -> float:
    ...
    bonus = 0.02 + 0.01 * float(
        (getattr(agent, "realized", None) or {}).get("survive_by_motif", {}).get(mid, 0)
        or 0
    )
...
def macro_increase_path(
    ...
    *,
    ...
    for depth in range(depth_lo, min(depth_hi, max(int(max_depth), 1) + 1)):
        ...
        siblings.sort(key=lambda a: -_survive_rank(agent, a))
        ...
        for alt in siblings:
            ...
            zone = "void_seek"
            if alt.region == MacroRegion.Rim:
                zone = "cluster_edge"
            elif alt.region == MacroRegion.Sheet:
            ...
            reward += 0.01 * _survive_rank(agent, alt)
            ...
            for join_step in join_nbrs:
                ...
                if dg is not None and hasattr(dg, "realized") and dg.realized(join_step):
                    chain_bonus = max(
                        chain_bonus,
                        0.015 + 0.005 * _survive_rank(agent, alt),
                    )
            if chain_bonus > 0.0:
                reward += chain_bonus
                path_step_join += 1
